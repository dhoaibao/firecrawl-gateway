...
/** Replaces documented async IDs, including the scrape session ID nested in response metadata. */
export function virtualizeCreationResponse(body: Buffer, publicJobId: string, publicUrl: string): VirtualizedCreationResponse | null {
  ...
  if (typeof record.id === "string" && record.id.length > 0) {
    const upstreamJobId = record.id;
    record.id = publicJobId;
    if (typeof record.url === "string") record.url = publicUrl;
    return { body: Buffer.from(JSON.stringify(record)), upstreamJobId };
  }
