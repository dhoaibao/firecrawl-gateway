...
| `PORT` | Optional for a direct Node process; set by Compose | `8080` | TCP port the Express server listens on. Compose always injects `8080`; use this rather than `GATEWAY_PORT` when not using Compose. |
| `WORKER_ENABLED` | Optional for a direct Node process; overridden by Compose | `true` | Starts in-process background jobs when true. Compose forces `false` for `api` and `true` for the dedicated `worker`, so do not override it there. |
...
| `TRUST_PROXY` | Optional | `false` | Express proxy-trust setting. Enable only behind a known reverse proxy that correctly sanitizes forwarded headers; otherwise client-supplied forwarding headers can affect request metadata. |
